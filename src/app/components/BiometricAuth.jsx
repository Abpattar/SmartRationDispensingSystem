...
export function BiometricAuth() {
  ...
  return (<div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <AnimatedContent distance={50} delay={0.1} direction="horizontal" reverse={false}>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="Enter beneficiary ID or scan fingerprint" value={beneficiaryId} onChange={(e) => setBeneficiaryId(e.target.value)} disabled={authStep === "scanning"} />
              </div>

              {/* Scanner Display */}
              <div className="relative">
                <div className={`border-4 rounded-2xl p-8 flex items-center justify-center aspect-square transition-all ${authStep === "idle" ? "border-gray-300 bg-muted/50" :
                  authStep === "scanning" ? "border-blue-500 bg-blue-500/10 animate-pulse" :
                    authStep === "success" ? "border-green-500 bg-green-500/10" :
                      "border-red-500 bg-red-50"}`}>
                  {authStep === "idle" && (<div className="text-center">
                  ...
                  {authStep === "scanning" && (<div className="text-center">
                  ...
                  {authStep === "success" && (<div className="text-center">
                  ...
                  {authStep === "failed" && (<div className="text-center">
              ...
              <div className="flex gap-3">
                {(authStep === "idle" || authStep === "failed") && (<Button onClick={handleBiometricScan} className="flex-1 bg-blue-600 hover:bg-blue-700">
                  <Fingerprint className="w-4 h-4 mr-2" />
                  Start Biometric Scan
                </Button>)}

                {(authStep === "success" || authStep === "failed") && (<Button onClick={resetAuth} variant="outline" className="flex-1">
      ...
      <AnimatedContent distance={50} delay={0.15} direction="horizontal" reverse={true}>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            {authStep === "success" ? (<div className="space-y-4">
              <div className="flex items-start gap-4 p-4 bg-green-500/10 rounded-lg border border-green-200">
                <img src={beneficiaryDetails.photo} alt="Beneficiary" className="w-20 h-20 rounded-lg" />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(beneficiaryDetails.remainingQuota).map(([item, quantity]) => (<div key={item} className="p-3 bg-blue-500/10 rounded-lg border border-blue-200">
    ...
    <AnimatedContent distance={50} delay={0.2}>
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentVerifications.map((verification, index) => (<div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${verification.status === "verified" ? "bg-green-500/100" : "bg-red-500"}`}></div>
                <div>
